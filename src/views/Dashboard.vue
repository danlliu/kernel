<template>
  <div class="w-100">
    ...
    >
      ...
      <div
        class="col-3 hideable"
        v-bind:class="isAdding && 'hidden'"
        id="buttonDiv"
      >
        <button
          class="btn btn-lg btn-primary w-100 m-1"
          v-bind:disabled="isAdding"
          @click="startBreak"
        >
          break time!
        ...
        <button
          class="btn btn-lg btn-secondary w-100 m-1"
          v-bind:disabled="isAdding"
          @click="modals['finishDayModal'].show()"
        >
          finish today
        </button>
      </div>
    ...
    <div id="newTaskForm" v-bind:class="isAdding || 'hidden'">
      <h3>add a task</h3>
      <button
        class="btn btn-close position-absolute"
      ...
      <form @submit.prevent="addTask">
        ...
        <div class="input-group form-floating mb-0">
          ...
          <div class="btn-group">
            ...
            >
              <span v-if="addingTask.tag === ''" class="badge bg-secondary"
                >no tag</span
              >
              <span
                v-else
            ...
            <div
              class="dropdown-menu"
              style="overflow-y: auto; max-height: 768%"
            >
              ...
              <a
                ...
                @click="
                  () => {
                    addingTask.tag = tag.tag;
                    addingTask.tagstyle = tag.style;
                  }
                "
              >
        ...
        <div class="input-group mt-0">
          <small class="form-label"
            >or
            <a class="link-primary" @click="modals['addTagModal'].show()"
              >add a tag</a
            ></small
          >
        </div>
    ...
    >
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>
              Congrats on making it through the day! Do you wish to finish for
              today? This will hide all tasks you've marked as finished in your
              task list. You'll always be able to go back and work on more tasks
              later, but the ones currently marked as finished won't be
              available.
            </p>
          </div>
          ...
          <div class="modal-body">
            <div v-for="task in tasks" class="tasklist-item" :key="task.id">
              <div v-if="task.tag === 'break'">
                <h2>
                  Time Spent Chilling: {{ timeElapsedString(task.timespent) }}
                </h2>
              </div>
            </div>
          </div>
    ...
    <router-link
      class="btn badge rounded-pill bg-info bi-graph-up hover-button"
      style="
        font-size: 1.5rem;
        position: fixed;
        right: 32px;
...
export default {
  ...
  methods: {
    changeTimer: function (data) {
      if (this.runningId !== data) {
        //something else is running, start running
        this.$emit("clearTimer", this.runningId);
        this.runningId = data;
      } else {
        //find the one thats running and pause it
        for (let i in this.tasks) {
          if (this.tasks[i].id === data) {
            if (data === this.runningId) {
              //stop timer.
              this.$emit("clearTimer", this.runningId);
              this.runningId = null;
            }
